 ...
 */
var _transformAttribute = ('transform' in document.documentElement.style) ? 'transform' : undefined;
_transformAttribute = _transformAttribute || (('-webkit-transform' in document.documentElement.style) ? '-webkit-transform' : 'undefined');
_transformAttribute = _transformAttribute || (('-moz-transform' in document.documentElement.style) ? '-moz-transform' : 'undefined');
_transformAttribute = _transformAttribute || (('-ms-transform' in document.documentElement.style) ? '-ms-transform' : 'undefined');
_transformAttribute = _transformAttribute || (('-o-transform' in document.documentElement.style) ? '-o-transform' : 'undefined');

function setAbsoluteSizeAndPosition(elm, left, top, width, height) {
    elm.setAttribute('style', 'width: ' + width + 'px; height: ' + height + 'px; ' + _transformAttribute + ': translate3d(' + left + 'px, ' + top + 'px, 0px);');
}
...
autoLayout(document.getElementById('right'), exampleEVFL);
